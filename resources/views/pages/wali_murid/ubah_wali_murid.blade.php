    ...
    <form action="{{ route('wali-murid.update', $data->id) }}" method="POST">
        ...
        <div class="card-body">
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="nama_wali" id="nama_wali" value="{{ $data->nama_wali }}" placeholder="Masukan Nama Wali Murid">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="id_siswa" name="id_siswa">
                    ...
                    @foreach ($siswa as $item)
                        <option value="{{ $item->id }}"  {{ ( $item->id == $data->id_siswa) ? 'selected' : '' }}>{{ $item->nama_siswa }}</option>
                    @endforeach
                </select>
            </div>
        </div>
